...
* 모델: claude-haiku-4-5 (가벼운 설명용)
* anthropic SDK 미설치 시 pip install anthropic
"""
...
def generate_explanation(timetable, scores, user_pref, model="claude-haiku-4-5"):
    """LLM을 호출해 추천 이유 문장을 반환."""
    try:
        from anthropic import Anthropic
    except ImportError:
        return "(anthropic 패키지가 설치되어 있지 않아요. `pip install anthropic` 필요)"

    if not os.environ.get("ANTHROPIC_API_KEY"):
        return "(ANTHROPIC_API_KEY 환경변수가 설정되어 있지 않아요)"
