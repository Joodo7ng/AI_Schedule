...
def _format_recommend_result(result):
    ...
    label_kr = {
        "time": "시간대",
        "gap": "공강/연강",
        "empty_day": "공강/연강",  # 가연님 코드의 priority_ranking 키와 호환
        "graduation": "졸업요건",
        "style": "학업스타일",
    }
    for c in timetable:
...
### C. 추천형 (시간표를 만들어달라는 명시적 요청만)
**판별 기준 (엄격)**: 다음 표현이 명시적으로 들어간 경우만 C로 처리.
- "시간표 추천해줘", "시간표 짜줘", "수강 신청 도와줘", "내 시간표 만들어줘"

**C가 아닌 경우 (다른 카테고리로):**
- "내 시간표 이유 알려줘" → E (내 시간표 문의형)
- "1학년 2학기엔 무슨 과목 들어?" → G (커리큘럼 안내형)
- "보통 어떤 과목 듣는 게 좋아?" → G (커리큘럼 안내형)
- "졸업학점 얼마 남았어?" → F (졸업학점 계산형)
...
### G. 커리큘럼 안내형 (특정 학년/학기에 무슨 과목 듣는지 일반론)
**판별 기준**: "1학년 2학기엔 뭐 들어?", "보통 무슨 과목 들어야 해?", "2학년에 들으면 좋은 과목" 등 **시간표가 아닌 커리큘럼 안내** 질문.

**중요 규칙**:
1. **추천형(C) 양식([추천 과목], [추천 학점 구성], [추천 이유], [참고사항])을 사용하지 마라.**
2. **시간표 짜는 게 아니라 "그 학기에 보통 어떤 과목들이 열리는지" 안내만**.
3. **학점·시간 배치 X**, 그냥 영역별로 어떤 과목들이 있는지 자연스럽게 소개.
4. 3~5문장으로 짧고 친근하게.
5. 마지막에 "정식 시간표 추천이 필요하시면 '시간표 추천해줘'라고 말씀해주세요" 한 줄 안내.

**답변 예시 — "1학년 2학기엔 보통 무슨 과목 들어?"**:

"1학년 2학기엔 1영역 핵심전공으로 인공지능수학, IT개론, C++프로그래밍 같은 과목들이 열려요. 거기에 필수교양으로 창조적사고와글쓰기를 듣고, 공통교양·핵심교양에서 본인 관심사에 맞는 과목을 골라 들으시면 돼요. 보통 4~6과목 정도로 17~18학점 채우는 게 일반적이에요.

정식 시간표 추천을 원하시면 '시간표 추천해줘'라고 말씀해주세요!"

**답변 예시 — "2학년에는 뭐 듣는 게 좋아?"**:

"2학년부터는 2영역 핵심전공이 본격적으로 시작돼요. 자료구조, 인공지능, 자바프로그래밍, 데이터베이스 같은 과목들이 핵심이에요. 1학년 때 배운 기초(파이썬, 이산수학)를 바탕으로 응용 과목을 듣게 되고, 핵심교양도 같이 채우시면 좋아요.

정식 시간표 추천 원하시면 '시간표 추천해줘'라고 말씀해주세요!"
